...
namespace _Scripts.Gameplay.Actions
{
    ...
    public class ActionVisualization
    {
        ...
        private PrevisualizationActionFX _previsualizationActionFX = null;
        ...
        public void Update()
        {
            ...
            if (_previsualizationActionFX != null)
            {
                _previsualizationActionFX.Update();
            }
        }

        //helper wrapper for a FindIndex call on m_PlayingActions.
        private List<int> FindAction(string actionName, bool anticipatedOnly)
        {
            var indexes = new List<int>();
            for (var actionIndex = 0; actionIndex < m_PlayingActions.Count; actionIndex++)
            {
                var action = m_PlayingActions[actionIndex];
                if (action.Description.ActionName == actionName && (!anticipatedOnly || action.Anticipated))
                {
        ...
        public void StartActionsAtState(ref ActionRequestData data, ActionPlayer.ActionState state)
        {
            var description = GameDataSource.Instance.ActionDataByName[data.ActionName];

            foreach (var actionFxDescription in description.Spawns)
            {
                if (actionFxDescription.ActionStateStart == state)
                {
                    PlayAction(ref data, actionFxDescription);
                }
            }
        }

        public void PlayAction(ref ActionRequestData data, ActionDescription.ActionFXDescription actionFxDescription)
        {
            // check if action has anticipated actions
            var anticipatedActionIndex = FindAction(data.ActionName, true);
            
            if (anticipatedActionIndex.Count > 0)
            {
                foreach (var index in anticipatedActionIndex)
                {
                    m_PlayingActions.RemoveAt(index);
                }
            }
            else
            {
                var actionFX = ActionFX.MakeActionFX(ref data, Parent, actionFxDescription);
        ...
        // End all actionsFX of the requested action that are not supposed to be played at this state 
        public void EndActionsAtState(ref ActionRequestData data, ActionPlayer.ActionState state)
        {
            var description = GameDataSource.Instance.ActionDataByName[data.ActionName];
            for (var i = m_PlayingActions.Count - 1; i >= 0; i--)
            {
                var action = m_PlayingActions[i];
                if (
                    description.ActionName == action.Description.ActionName 
                    && action.ActionFXDescription.ActionStateStart != state
                    && action.ActionFXDescription.IsInterruptible
                    )
                {
                    CancelActionAtIndex(i);
                }
            }
        }

        public void CancelAction(ref ActionRequestData data)
        {
            var actionIndexes = FindAction(data.ActionName, false);
            foreach (var actionIndex in actionIndexes)
            {
                // check if action FX can be interrupted
                if (m_PlayingActions[actionIndex].ActionFXDescription.IsInterruptible)
                {
                    CancelActionAtIndex(actionIndex);
                }
            }
        }

        public void CancelActionAtIndex(int actionIndex)
        {
            var actionFX = m_PlayingActions[actionIndex];
            actionFX.Cancel();
            m_PlayingActions.RemoveAt(actionIndex);
        }

        public void SetPrevisualization(ref ActionRequestData data)
        {
            var actionFXDescription = new ActionDescription.ActionFXDescription();
            
            _previsualizationActionFX = new PrevisualizationActionFX(ref data, Parent, actionFXDescription);
            _previsualizationActionFX.Activate();
        }
        ...
        public void CancelPrevisualization()
        {
            _previsualizationActionFX?.Cancel();
            _previsualizationActionFX = null;
        }

        public void CancelAllActionsOfType(string actionName)
        {
            for (int i = m_PlayingActions.Count - 1; i >= 0; --i)
            {
                if (m_PlayingActions[i].Description.ActionName == actionName)
                {
