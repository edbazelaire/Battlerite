...
namespace _Scripts.Gameplay.ConnectionManagement
{
    ...
    public class ServerGameNetPortal : MonoBehaviour
    {
        ...
        public bool AreAllClientsInServerScene()
        {
            foreach (var kvp in m_ClientSceneMap)
            {
                if (kvp.Value != ServerScene) {
                    Debug.Log($"client {kvp.Key} in scene {kvp.Value} and not in scene {ServerScene}");
                    return false;
                }
            }
        ...
        void ApprovalCheck(NetworkManager.ConnectionApprovalRequest request, NetworkManager.ConnectionApprovalResponse response)
        {
            ...
            print("Approval check for : " + clientId);
           ...
           if (clientId == NetworkManager.Singleton.LocalClientId)
            {
                SessionManager<SessionPlayerData>.Instance.SetupConnectingPlayerSessionData(
                    clientId, 
                    m_Portal.GetPlayerId(),
                    new SessionPlayerData(clientId, m_Portal.PlayerName, m_Portal.AvatarRegistry.GetRandomAvatar().Guid.ToNetworkGuid(), 0, 0, true)
                    );
        ...
        void ServerStartedHandler()
        {
            ...
            SceneLoaderWrapper.Instance.LoadScene(ScenesEnum.CHAR_SELECT, useNetworkSceneManager: true);
        }
