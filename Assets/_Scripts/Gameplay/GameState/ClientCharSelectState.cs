...
namespace Unity.Multiplayer.Samples.BossRoom.Client
{
    ...
    public class ClientCharSelectState : GameStateBehaviour
    {
        ...
        [Header("Lobby Seats")]
        [SerializeField]
        [Tooltip("Collection of portrait-boxes, one for each potential lobby member")]
        List<UICharSelectPlayerSeat> m_PlayerSeats;
        public List<UICharSelectPlayerSeat> PlayerSeats => m_PlayerSeats;

        [Tooltip("Representational information for each player")]
        public Color[] m_IdentifiersForEachPlayerNumber;
        ...
        Dictionary<Guid, GameObject> m_SpawnedCharacterGraphics = new ();
        ...
        void UpdateCharacterSelection(CharSelectData.SeatState state, int seatIdx = -1)
        {
            ...
            else
            {
                if (seatIdx != -1)
                {
                    ...
                    if (isNewSeat) {
                        var selectedCharacterGraphics = GetCharacterGraphics(m_PlayerSeats[seatIdx].avatar);
