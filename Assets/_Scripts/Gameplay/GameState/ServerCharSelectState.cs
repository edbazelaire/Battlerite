...
namespace _Scripts.Gameplay.GameState
{
    ...
    public class ServerCharSelectState : GameStateBehaviour
    {
            ...
            else
            {
                ...
                foreach (CharSelectData.LobbyPlayerState playerInfo in CharSelectData.LobbyPlayers)
                {
                    if (playerInfo.ClientId != clientId && playerInfo.SeatIdx == newSeatIdx && playerInfo.SeatState == CharSelectData.SeatState.LockedIn)
                    {
                        ...
                        CharSelectData.LobbyPlayers[idx] = new CharSelectData.LobbyPlayerState(
                            clientId,
            ...
            foreach (CharSelectData.LobbyPlayerState playerInfo in CharSelectData.LobbyPlayers)
            {
                ...
                if (playerNetworkObject && playerNetworkObject.TryGetComponent(out PersistentPlayer persistentPlayer))
                {
                    ...
                    persistentPlayer.NetworkAvatarGuidState.AvatarGuid.Value =
                        ClientCharSelectState.Instance.PlayerSeats[playerInfo.SeatIdx].avatar.Guid.ToNetworkGuid();
                }
            }
        }
        ...
        IEnumerator WaitToEndLobby()
        {
            ...
            SceneLoaderWrapper.Instance.LoadScene(ScenesEnum.ARENA1, useNetworkSceneManager: true);
        }
        ...
        void SeatNewPlayer(ulong clientId)
        {
            ...
            if (sessionPlayerData.HasValue)
            {
                ...
                // TODO : CHANGE NAME in PlayerSessionData =============================================
                // user is currently saved with a bad empty name. We need to implement name changing option + check
                // name during inscription
                string name = sessionPlayerData.Value.PlayerName;
                ...
                CharSelectData.LobbyPlayers.Add(new CharSelectData.LobbyPlayerState(clientId, name, playerData.PlayerNumber, CharSelectData.SeatState.Inactive));
